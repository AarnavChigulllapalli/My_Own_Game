class Game {
  ...
  start() {
    ...
    form = new Form();
    form.display();
    var rand=Math.round(random(-500,-750))
    ...
    this.addSprites(teleport_pad, 4, teleport_padImage, 0.5);
  ...
  handleTeleportPad(index) {
    // Adding fuel
    cars[index - 1].overlap(teleport_pad, function(collector, collected) {
  ...
  showRank() {
    swal({
      title: `Awesome!${"\n"}Rank${"\n"}${player.rank}`,
      text: "If you are 1st you win and if you are 2nd you lose",
      imageUrl:
  ...
  gameOver() {
    swal({
      title: `Game Over`,
      text: "Oops you lost the race and $1M",
      imageUrl:
